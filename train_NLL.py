...
def main():
    parser = argparse.ArgumentParser(
        description="Train protein language model on DMS data with LoRA"
    )
    ...
    x, _ = create_fold_split(dataset_train)
    ...
    # (Trainer will handle train/eval mode switching, but we set eval here for initial state)
    model.eval()
    ...
    # Verify LoRA weights are zero-initialized (should be close to zero)
    # This ensures evaluation at step 0 matches zero-shot
    if hasattr(base_model, 'peft_config'):
        print("\nVerifying LoRA initialization...")
        for name, param in base_model.named_parameters():
            if 'lora' in name.lower() and param.requires_grad:
                max_val = param.abs().max().item()
                if max_val > 1e-6:
                    print(f"  WARNING: LoRA param {name} has non-zero init: {max_val}")
                else:
                    print(f"  ✓ LoRA param {name} is zero-initialized (max={max_val:.2e})")
                break  # Just check first LoRA param
